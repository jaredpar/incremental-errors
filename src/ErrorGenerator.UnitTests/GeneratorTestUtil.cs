...
public sealed class GeneratorTestUtil
{
    ...
    {
        Generator = generator.AsSourceGenerator();
    }

    public GeneratorTestUtil(ISourceGenerator generator)
    {
        Generator = generator;
    }

    private Compilation GetCompilation(string sourceCode) =>
        GetCompilation([CSharpSyntaxTree.ParseText(sourceCode)]);
        ...
        int generatedTreeIndex = 0)
    {
        ...
        var actualGeneratedCode = generatedTree.ToString();
        Assert.Equal(Trim(expectedGeneratedCode), Trim(actualGeneratedCode));
    }
